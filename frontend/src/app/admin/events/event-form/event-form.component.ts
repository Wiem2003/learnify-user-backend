...
@Component({
            ...
            <div class="form-group">
              ...
              @if (photoPreview) {
                <button type="button" class="btn-remove-photo" (click)="removePhoto()">
                  <i class="ti ti-x"></i> Remove photo
                </button>
              }
            </div>
          </div>
  ...
  styles: [`
    ...
    .photo-upload-area { border: 2px dashed rgba(61,61,96,0.2); border-radius: 12px; padding: 24px; text-align: center; cursor: pointer; transition: border-color 0.2s; min-height: 160px; display: flex; align-items: center; justify-content: center; overflow: hidden; &:hover { border-color: var(--color-primary); } }
    .photo-placeholder { display: flex; flex-direction: column; align-items: center; gap: 8px; color: var(--color-gray-500); i { font-size: 40px; } span { font-size: 14px; font-weight: 600; } small { font-size: 12px; } }
    .photo-preview { max-width: 100%; max-height: 200px; border-radius: 8px; object-fit: cover; }
    .btn-remove-photo { margin-top: 8px; display: inline-flex; align-items: center; gap: 4px; padding: 6px 12px; border-radius: 8px; font-size: 12px; font-weight: 600; border: 1px solid rgba(220,53,69,0.3); color: #dc3545; background: transparent; cursor: pointer; &:hover { background: rgba(220,53,69,0.08); } }
...
export class EventFormComponent implements OnInit {
    ...
    if (id) {
      ...
      this.eventService.getById(this.eventId).subscribe({
        next: (event: any) => {
          ...
          if (event.photoUrl) {
            this.photoPreview = event.photoUrl.startsWith('http') ? event.photoUrl : this.backendUrl + event.photoUrl;
          }
        }
      });
    }
  ...
  onSubmit(): void {
    ...
    fd.append('placesLimit', String(Number(formData.placesLimit)));
    fd.append('description', formData.description);
    fd.append('organizerFirstName', formData.organizerFirstName || 'Admin');
    fd.append('organizerLastName', formData.organizerLastName || 'User');
    if (this.selectedPhoto) {
      fd.append('photo', this.selectedPhoto);
    }

    if (this.isEditMode && this.eventId != null) {
      this.eventService.update(this.eventId, fd).subscribe({
        next: () => this.router.navigate(['/admin/events']),
        error: (err: any) => console.error('Update event error', err)
      });
    } else {
      this.eventService.create(fd).subscribe({
        next: () => this.router.navigate(['/admin/events']),
