...
import pi.backrahma.Repository.ReservationRepository;
...
public class ReservationService {
    ...
    public byte[] generateQRCodeImage(Long reservationId) {
        Reservation reservation = reservationRepository.findById(reservationId)
                .orElseThrow(() -> new ReservationException("Réservation introuvable"));
        
        try {
            return pdfTicketService.generateQRCodeOnly(reservation.getTicketCode());
        } catch (Exception e) {
            throw new ReservationException("Erreur lors de la génération du QR Code: " + e.getMessage());
        }
    }
    ...
    public pi.backrahma.dto.TicketValidationResponse validateTicket(String ticketCode) {
        Reservation reservation = reservationRepository.findByTicketCode(ticketCode)
                .orElse(null);

        if (reservation == null) {
            return pi.backrahma.dto.TicketValidationResponse.builder()
                    .valid(false)
